...
def t0erm_cbv_evaluate0(term: t0erm) -> t0erm:
    if False:
        ...
        v1 = t0erm_cbv_evaluate0(term.arg1)
        v2 = t0erm_cbv_evaluate0(term.arg2)
        return T0Mpair(v1, v2)
    elif isinstance(term, T0Mpfst):
        v1 = t0erm_cbv_evaluate0(term.arg1)
        if isinstance(v1, T0Mpair):
            return v1.arg1
        else:
            raise TypeError(f"t0erm_cbv_evaluate0: fst expects a pair ({v1})")
    elif isinstance(term, T0Mpsnd):
        v1 = t0erm_cbv_evaluate0(term.arg1)
        if isinstance(v1, T0Mpair):
            return v1.arg2
        else:
            raise TypeError(f"t0erm_cbv_evaluate0: snd expects a pair ({v1})")
    else:
        raise TypeError(f"t0erm_cbv_evaluate0({term})")        
#
########################################################################
